...
class TestWidgetsAndSlides(MpfIntegrationTestCase, MpfSlideTestCase):
    ...
    def test_moving_slide_frame(self):
        ...
        self.assertAlmostEqual(16, self.mc.active_slides['top_slide'].children[0].widget.y)

        self.post_event("move_out")
        self.advance_time_and_run()
        self.assertAlmostEqual(-100.0, self.mc.active_slides['top_slide'].children[0].widget.y)
        self.post_event("move_back")
        self.advance_time_and_run()

        self.assertAlmostEqual(-134.0, self.mc.active_slides['top_slide'].children[0].widget.y)
