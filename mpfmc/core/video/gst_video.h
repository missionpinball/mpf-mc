...
static GstFlowReturn c_on_appsink_sample_alpha_channel(GstElement *appsink, callback_data_t *data)
{
	GstSample *sample = NULL;
	GstBuffer *buffer = NULL;
	GstMapInfo mapinfo;
	...
	structure = gst_caps_get_structure(caps, 0);
	g_warning(gst_caps_to_string(caps));
	gst_structure_get_int(structure, "width", &width);
	gst_structure_get_int(structure, "height", &height);
    ...
    // Because buffer width is always a multiple of 4 (4 channels: rgba) we can directly use the buffer in memory
    cbuffer = (gchar *)mapinfo.data;
    size = mapinfo.size;
...
static gulong c_appsink_set_sample_callback(GstElement *appsink, appcallback_t callback, PyObject *userdata, int alpha_channel)
{
    ...
    if (alpha_channel)
        return g_signal_connect_data(
                appsink, "new-sample",
                G_CALLBACK(c_on_appsink_sample_alpha_channel), data,
...
static void c_appsink_pull_preroll(GstElement *appsink, appcallback_t callback, PyObject *userdata, int alpha_channel)
{
	...
	if (alpha_channel)
	    c_on_appsink_sample_alpha_channel(appsink, &data);
	else
